...
; (function ($, window, document, undefined) {
    // В обработчике window.onload иногда работает логика, завершения которой нужно дождаться прежде чем выполнять свою,
    // поэтому дополняем уже заданную логику window.onload вызовом соответствующего события.
    var originalOnloadMethod = window.onload;
    window.onload = function () {
        if (typeof originalOnloadMethod === 'function') {
            originalOnloadMethod.apply(this, arguments);
        }

        window.onLoadIsFinished = true;
        $(window).trigger('onfinishload');
    };
    ...
    function parseQuery(query) {
        ...
        var decode = function (encodedString) {
            ...
            try {
                decodedString = (decodeURIComponent || unescape)(encodedString);
            } catch (exc) {
    ...
    $.ics = $.extend($.ics, {
        ...
        dialog: {
            ...
            modal: function (options) {
                ...
                var $dialogLoadingSpinner;
                var $dialog = modalContent.dialog(options, {
                    open: function () {
                        if ($dialogLoadingSpinner) {
                            $dialogLoadingSpinner.remove();
                        }
                ...
                // Для правильного позиционирования модального окна, и предотвращения его смещения после открытия,
                // открываем его только после того как завершится логика обработчика window.onload.
                // Например при включенной настройке MaintainScrollPositionOnPostBack в window.onload происходит восстановление прежних позиций скроллов на странице
                // (см. https://msdn.microsoft.com/en-us/library/system.web.ui.page.maintainscrollpositiononpostback(v=vs.110).aspx).
                if (window.onLoadIsFinished) {
                    $dialog.dialog('open');
                } else {
                    $dialogLoadingSpinner = $('<div class="loading-modal"><i class="fa fa-spinner fa-spin fa-3x"></i></div>');
                    $dialog.closest('body').append($dialogLoadingSpinner);

                    $(window).on('onfinishload', function () {
                        $dialog.dialog('open');
                    });
                }
                ...
                var ret = {
                    ...
                    context: options.href
                        ? $('.ics-modal-iframe')[0].contentWindow
                        : window
                };
